...
def two_snakes(image_size, seed_distance,
    ...
    for isegment in range(1):
        ...
        current_image, current_mask, current_segment_masks, current_pivots, current_orientations, origin_centers, success \
        = initialize_two_seeds(image_size, seed_distance,
    ...
    for isegment in range(num_segments-1):
        ...
        for isnake in range(len(current_segment_masks)):
            current_image, current_mask, current_segment_masks[isnake], current_pivots[isnake], current_orientations[isnake], terminal_centers[isnake], success \
    ...
    if display_snake:
        ...
        plt.show()
    return current_image, current_mask, origin_centers, terminal_centers, True


def initialize_two_seeds(image_size, seed_distance,
    ...
    while trial_count <= max_segment_trial:
        ...
        sampled_center1 = [np.round_(float(sampled_head1[0]+sampled_tail1[0])/2),
                           np.round_(float(sampled_head1[1]+sampled_tail1[1])/2)]
    ...
    while trial_count <= max_segment_trial:
        ...
        # generate dilation struct
        _, struct = snakes.draw_line_n_mask(struct_shape, struct_head, sampled_orientation_in_rad2, length, thickness, margin,
                                     large_dilation_structs, aa_scale)
        # head-centric struct

        # dilate mask using segment
        lined_mask = np.maximum(mask, m_im1).copy()
        lined_mask[:seed_distance*2,:] = 1
        lined_mask[image_size[0]-seed_distance*2:,:] = 1
        lined_mask[:,:seed_distance*2] = 1
        lined_mask[:,image_size[1]-seed_distance*2:] = 1
        ...
        sampled_center2= [int(np.round_(float(sampled_head2[0] + sampled_tail2[0]) / 2)),
                          int(np.round_(float(sampled_head2[1] + sampled_tail2[1]) / 2))]
    ...
    return image, mask, [m_im1,m_im2], [sampled_pivot1, sampled_pivot2], [sampled_orientation_in_rad1, sampled_orientation_in_rad2], [sampled_center1, sampled_center2], True
...
def test():
    ...
    snake_length = 9 # from 5 to 21, with steps of 4
    ...
    while not success:
        ...
        two_snakes([imsize, imsize], seed_distance,
