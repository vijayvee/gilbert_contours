...
class Args:
    def __init__(self,
                 ...
                 random_contrast=False, pause_display=False, distractor_contrast=1.0,
...
# contrast_range (min-max only active when randomcontrast is true)
# dist_uniform (sample from uniform vs normal. False by default)
# scale_contrast (std of the dist)
# random_contrast (T/F)
args.random_contrast = True
for abs_contrast in [0.2,0.4,0.8,1.0]:
    args.contrast_range = [-abs_contrast, abs_contrast]
    dataset_subpath = 'scale_contrast_' + str(abs_contrast) #TODO: TRUNCATED NORMAL?
    args.contour_path = os.path.join(dataset_root, dataset_subpath)
    gilbert_contours.from_wrapper(args)
args.random_contrast = False
args.contrast_range = [0.7,1.0]
